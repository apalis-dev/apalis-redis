...
-- ARGV[1]: the idempotency_ttl
-- ARGV[2]: emit pubsub events
-- ARGV[3..]: [task_id1, task_data1, attempts1, max_attempts1, metadata_json1, schedule_time1, idempotency_key1,
    ...
    if task_id and task_data then
        ...
        if idempotency_key and #idempotency_key > 0 then
            ...
            if claimed == 0 then
                ...
                if idempotency_ttl and #idempotency_ttl > 0 then
                    redis.call("expire", idem_field, idempotency_ttl)
                end
            end
        end
        ...
        if allowed then
            ...
            if set == 1 then
                ...
                -- Base metadata
                local meta = {"attempts", tonumber(attempts), "max_attempts", tonumber(max_attempts), "status",
                              "Pending"}
